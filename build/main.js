...
class Sonos extends utils.Adapter {
    ...
    async ensureHomeTheaterState() {
        ...
        await this.setStateAsync('home_theater', (0, home_theater_1.homeTheaterStateJson)(this.htBonds), true);
    }
    ...
    async takeSonosState(ip, sonosState) {
        ...
        await this.setState({ device: 'root', channel: ip, state: 'current_metadata' }, { val: resume.metadata, ack: true });
        // elapsed time
        ...
        await this.setState({ device: 'root', channel: ip, state: 'current_duration_s' }, { val: toFormattedTime(duration), ack: true });
        // Track number
    ...
    async copyPlaybackToGroupMembers(coordinatorIp, sonosState, ps, coverUrl, display) {
        ...
        for (const memberIp of members) {
            ...
            await this.setState({ device: 'root', channel: memberIp, state: 'current_uri' }, { val: memberResume.uri, ack: true });
            await this.setState({ device: 'root', channel: memberIp, state: 'current_metadata' }, { val: memberResume.metadata, ack: true });
