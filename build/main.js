...
class Sonos extends utils.Adapter {
        ...
        if (id.state === 'state_simple') {
            ...
            if (!favorite) {
                ...
                promise = player
                    ...
                    .then(() => player.play())
                    .then(async () => {
                    await this.setState({ device: 'root', channel: id.channel, state: 'current_album' }, { val: favorite, ack: true });
                    await this.setState({ device: 'root', channel: id.channel, state: 'current_artist' }, { val: favorite, ack: true });
                })
            ...
            if (!playlist) {
                ...
                promise = player
                    ...
                    .then(() => player.play())
                    .then(async () => {
                    await this.setState({ device: 'root', channel: id.channel, state: 'current_album' }, { val: playlist, ack: true });
                    await this.setState({ device: 'root', channel: id.channel, state: 'current_artist' }, { val: playlist, ack: true });
                })
    ...
    async takeSonosState(ip, sonosState) {
        ...
        if (sonosState.currentTrack.type === 'radio') {
            ...
            await this.setState({ device: 'root', channel: ip, state: 'current_station' }, { val: sonosState.currentTrack.stationName || '', ack: true });
        }
        else {
            await this.setState({ device: 'root', channel: ip, state: 'current_type' }, { val: sonosState.currentTrack.type === 'line_in' ? 2 : 0, ack: true });
            await this.setState({ device: 'root', channel: ip, state: 'current_station' }, { val: '', ack: true });
        }
        await this.setState({ device: 'root', channel: ip, state: 'current_title' }, { val: sonosState.currentTrack.title || '', ack: true });
        await this.setState({ device: 'root', channel: ip, state: 'current_album' }, { val: sonosState.currentTrack.album || '', ack: true });
        await this.setState({ device: 'root', channel: ip, state: 'current_artist' }, { val: sonosState.currentTrack.artist || '', ack: true });
        // elapsed time
    ...
    updateElapsed(ip) {
        ...
        void this.setState({ device: 'root', channel: ip, state: 'seek' }, { val: Math.round((channel.elapsed / channel.duration) * 1000) / 10, ack: true });
        void this.setState({ device: 'root', channel: ip, state: 'current_elapsed' }, { val: channel.elapsed, ack: true });
        void this.setState({ device: 'root', channel: ip, state: 'current_elapsed_s' }, { val: toFormattedTime(channel.elapsed), ack: true });
    }
    /**
