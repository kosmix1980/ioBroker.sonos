    ...
    vis.binds.sonos = {
                ...
                listHtml = recent.length
                    ? recent.map(function (item) {
                        return '<button type="button" class="sonos-ctrl-item" data-recent-uri="' + vis.binds.sonos.esc(item.uri || '') +
                            ...
                            '" data-recent-artist="' + vis.binds.sonos.esc(item.artist || '') +
                            '" data-recent-album="' + vis.binds.sonos.esc(item.album || '') +
                            '" data-recent-cover="' + vis.binds.sonos.esc(encodeURIComponent(item.cover || '')) +
                            '" data-recent-duration="' + vis.binds.sonos.esc(String(item.duration || '')) +
        ...
        bindUi: function ($div, selected, players, coordinator, mediaId) {
            ...
            $div.find('[data-recent-title]').on('click', function () {
                ...
                var artist = String($(this).attr('data-recent-artist') || '');
                var album = String($(this).attr('data-recent-album') || '');
                var duration = parseInt(String($(this).attr('data-recent-duration') || ''), 10) || 0;
                var metadata = '';
                var cover = '';
                try {
                ...
                try {
                    cover = decodeURIComponent(String($(this).attr('data-recent-cover') || ''));
                } catch (e) {
                    cover = '';
                }
                var sonosUri = /^(x-sonos|x-rincon-mp3radio|x-rincon-stream|x-rincon-cpcontainer|x-file-cifs|spotify:|file:|aac:|pndrradio:|https?:)/i.test(uri) &&
